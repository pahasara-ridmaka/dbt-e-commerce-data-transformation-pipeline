...
print(f"Users: {len(users)}, Products: {len(products)}, Promotions: {len(promotions)}, Orders: {len(orders)}")
...
with col1:
    ...
    revenue_by_date = filtered_data.groupby('order_date')['total_amount'].sum().reset_index()
    fig = px.line(
        revenue_by_date,
        x='order_date',
        y='total_amount',
...
with col2:
    ...
    revenue_by_category = filtered_data.groupby('category')['total_amount'].sum().reset_index()
    fig = px.pie(
        revenue_by_category,
        values='total_amount',
        names='category',
...
with col2:
    ...
    top_product_df = filtered_data.groupby('product_id')['total_amount'].sum().reset_index()
    top_product_df = top_product_df.merge(products[['product_id', 'product_name']], on='product_id', how='left')
    top_products = top_product_df.groupby('product_name')['total_amount'].sum().sort_values(ascending=True).head(10)
...
with col2:
    ...
    promo_usage['usage_count'] = [random.randint(0, 100) for _ in range(len(promotions))]
    fig = px.bar(
        promo_usage,
        x='promotion_code',
        y='usage_count',
...
with col3:
    ...
    signup_by_month = users.groupby(users['signup_date'].dt.to_period('M')).size().reset_index(name='count')
...
with col1:
    ...
    top_customers = filtered_data.groupby('user_id').agg({
        'total_amount': 'sum',
        'full_name': 'first'
...
with col2:
    ...
    # Product metrics
    product_metrics = filtered_data.groupby('product_id').agg({
        'total_amount': ['sum', 'mean', 'count']
    }).round(2)
    product_metrics.columns = ['Total Revenue', 'Avg Order Value', 'Number of Orders']
    product_metrics = product_metrics.sort_values('Total Revenue', ascending=False).head(10)
    ...
    st.dataframe(
        ...
        column_config={
            ...
            'Number of Orders': st.column_config.NumberColumn('Number of Orders', format='%d')
        }
    )
